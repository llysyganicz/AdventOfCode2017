...
let findLoop (banks: int[]) =
    let rec step states =
        ...
        if states |> List.exists(fun e -> Enumerable.SequenceEqual(e, banks)) then 
            let index = states |> List.findIndex(fun e -> Enumerable.SequenceEqual(e, banks))
            states.Length - index
        else step (states @ [(Array.toList banks)])
